        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-md-7">
                    ...
                    <form id="order-form" role="form" method="post" action="{{ route('order.store') }}"
                        ...
                        @if (!config('settings.social_mode'))
                            ...
                            @elseif(config('app.isqrsaas') && count($timeSlots) > 0)
                                ...
                                @if (config('settings.enable_pickup'))
                                    @if (
                                        (in_array('poscloud', config('global.modules', [])) || in_array('deliveryqr', config('global.modules', []))) &&
                                            $restorant->can_deliver == 1)
                                        <!-- We have POS in QR -->
...
@endsection
@section('js')
    <script async defer src="https://maps.googleapis.com/maps/api/js?key=<?php echo config('settings.google_maps_api_key'); ?>&callback=initAddressMap">
    </script>
    <!-- Stripe -->
