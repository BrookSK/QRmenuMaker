<?php
    namespace Modules\PrintTempl\Http\Controllers;
    ...
    class App
    {
        ...
        public function sendToPrintTempl($cmd,$file,$id,$token){
            $curl = curl_init();
            $postData=array();
            if(strlen($cmd)>5){
                $postData=array('printer_id' => $id,'raw_command' => $cmd);
            }else {
                $postData=array('printer_id' => $id,'pdf_url' => $file);
            }
            ...
            CURLOPT_URL => 'https://printtempl.com/api/v1/jobs',
